...
struct RiscvReg {
    ...
    bool dirty;       // whether it is out of sychronized with the memory（通常是寄存器被写）
...
class RiscvDesc : public MachineDesc {
  ...
  private:
    ...
    // translates a Alloc TAC into assembly instructions
    ...
    void emitLandTac(RiscvInstr::OpCode op, tac::Tac *t);
