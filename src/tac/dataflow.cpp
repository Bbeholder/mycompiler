...
void FlowGraph::analyzeLiveness(void) {
    ...
    // Step 1. computes Def and LiveUse 计算
    for (int i = 0; i < _n; ++i) {
...
void BasicBlock::analyzeLiveness(void) {
    ...
    for (t = t->prev; t != NULL; t = t->prev) {
        ...
        if (!t->LiveOut->contains(t->op0.var) && t->op_code!=Tac::CALL &&  t->op_code!=Tac::LOAD_SYMBOL && t->op_code!=Tac::STORE && t->op_code!=Tac::PARAM) {
            if (t == tac_chain) {
                tac_chain = t->next;
            } else {
                t->prev->next = t->next;
